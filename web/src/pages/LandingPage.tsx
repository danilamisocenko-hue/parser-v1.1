    ...
    <div onMouseMove={handleMouseMove} className="min-h-screen bg-[#000000] text-neutral-200 selection:bg-indigo-500/30 selection:text-indigo-200 relative overflow-hidden font-sans cursor-default sm:cursor-none">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 md:h-20 flex items-center justify-between">
          <div className="flex items-center gap-4 sm:gap-6">
            ...
            <div className="flex bg-white/5 border border-white/10 rounded-full p-0.5 sm:p-1 gap-0.5 sm:gap-1">
              <button className="bg-white text-black text-[8px] sm:text-[10px] font-bold px-2 sm:px-3 py-0.5 sm:py-1 rounded-full transition-colors">RU</button>
              <button className="text-neutral-400 hover:text-white text-[8px] sm:text-[10px] font-bold px-2 sm:px-3 py-0.5 sm:py-1 rounded-full transition-colors">EN</button>
            </div>
          </div>
          ...
          <div className="flex items-center justify-end gap-3 sm:gap-6">
            <button 
              ...
              className="text-xs sm:text-sm font-bold text-white hover:text-indigo-400 transition-colors bg-white/5 hover:bg-white/10 px-4 sm:px-5 py-1.5 sm:py-2 rounded-full border border-white/10"
            >
              Войти
            </button>
            <a href="#" className="text-sm font-medium text-neutral-500 hover:text-neutral-300 transition-colors hidden lg:block">Условия</a>
            <a href="#" className="text-sm font-medium text-neutral-500 hover:text-neutral-300 transition-colors hidden lg:block">Политика</a>
          </div>
        </div>
      </nav>
      ...
      <div id="bento" className="py-32 px-6 relative z-10 perspective-1000">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 auto-rows-auto md:auto-rows-[220px]">
             {/* Удобная панель */}
             <TiltCard onClick={(e) => {
               window.dispatchEvent(new CustomEvent('canvas-burst', { detail: { x: e.clientX, y: e.clientY, type: 'normal' } }));
             ...
             <TiltCard onClick={(e) => {
               window.dispatchEvent(new CustomEvent('canvas-burst', { detail: { x: e.clientX, y: e.clientY, type: 'normal' } }));
               setTimeout(() => openFeature('Скидки', 'Лучшие предложения для активных пользователей', 'Гибкая система скидок для продления подписки при долгосрочном сотрудничестве.'), 300);
             }} className="col-span-1 lg:col-span-2 row-span-1 p-8 flex flex-col text-left items-start justify-center overflow-hidden">
